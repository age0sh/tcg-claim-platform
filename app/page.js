...
// Las variables estáticas sí pueden ir afuera
const mockSellers = [
...
export default function Home() {
  ...
  // 🔥 AQUÍ ES DONDE DEBE VIVIR EL ESTADO DE PEDIDOS (Adentro de Home)
  const [pedidos, setPedidos] = useState([]);
  
  // Estado para el Objeto Usuario
      ...
      <main style={{ padding: "20px", maxWidth: "1200px", margin: "0 auto" }}>
        {currentTab === "inicio" && !selectedSeller && (
          <DashboardView 
            ...
            mercado={cartas} 
            pedidos={pedidos} // 🔥 Agregamos pedidos aquí para el dashboard
        ...
        {currentTab === "claims" && userRole === "comprador" && (
          <ClaimsView socket={socket} userId={user.id} cartas={cartas} pedidos={pedidos} />
        )}
        
        {currentTab === "claims" && userRole === "vendedor" && (
          <SellerMarketView socket={socket} userId={user.id} userName={user.name} mercado={cartas} />
        )}
        
        {currentTab === "calendar" && <CalendarView userId={user.id} userRole={userRole} userName={user.name} socket={socket} calendario={calendario} />}
        {currentTab === "profile" && <ProfileView userId={user.id} userName={user.name} userRole={userRole} socket={socket} pedidos={pedidos} />}
      </main>
